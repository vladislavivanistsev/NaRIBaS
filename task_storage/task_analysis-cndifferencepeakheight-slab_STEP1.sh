...
sed -i 's/cn_N(z)/cn_N^{\\sigma}(z)-cn_N^{PZC}(z)/g' $plotdata

# Edit the Matlab function to plot the difference with reference to the second column in the plotdata file
sed 's+SED_dir_scripts_SED+'$currentdir'/source+g' $currentdir/source/peakheight_cndifference.m > peakheight_cndifference.m
sed -i 's/SED_cnfile_SED/'$plotdata'/g' peakheight_cndifference.m
sed -i 's|SED_potentialdropfile_SED|'$dir_analysis'/'$fullpath2simulate'/potentialdrop/potentialdrop-smooth+'$temperature_name'.dat|g' peakheight_cndifference.m
sed -i 's/SED_savename_SED/numberdens-cn+'$numberdens_name'-'$electrode_name'+'$temperature_name'+difference+peakheight.dat/g' peakheight_cndifference.m
sed -i 's/SED_savename2_SED/numberdens-cn+'$numberdens_name'-'$electrode_name'+'$temperature_name'+difference+peakheight+differentialresponse.dat/g' peakheight_cndifference.m


# Run matlab
$matlabdir/matlab -nodisplay -nosplash -r  "peakheight_cndifference($ncols)"
