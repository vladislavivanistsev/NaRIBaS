...
# Replicas are already combined, no need for adding replica.list 
# Remove combination.list, this one will now be considered as input loop in the tasks
inputlists=(particle.list cation.list anion.list impurity.list temperature.list version.list boxsize.list duration_NPT.list rdf.list)
...
tasks=task_analysis-rdf-bulk-impurity_STEP3.sh
