...
temperature_name=$(echo ${current_temperature_T_REMD[0]} | awk '{print $1}')
version_name=$(echo ${current_version[0]} | awk '{print $1}')
surfacecharge_name=$(echo ${current_surfacecharge_T_REMD[0]} | awk '{print $1}')
...
fullpath_referencegro=$particle_name/$cation_name/$anion_name/$impurity_name/500/c/$surfacecharge_name/$replica_name
...
numofwalkers=$(cat $currentdir/$inputlists_folder/temperature_T_REMD.list | wc -l)
...
cp NVT.tpr $dir_experiments/$fullpath2simulate/NVT_$surfacecharge_name\_$runningnumber.tpr
...
pathtocopyback=$(pwd)
echo "cp NVT_$surfacecharge_name\_$runningnumber.tpr $pathtocopyback/NVT.tpr" >> $dir_experiments/$fullpath2simulate/copyback.txt
