...
do
	...
	legend=$legend' {\itT}='$temperature_name'K {\itU}_{drop}(V)'
	cp $plotdata tmp
	paste tmp 'capacitance+'$temperature_name'.dat' > $plotdata
	rm tmp
...
sed -i 's/SED_title_SED/''/g' basic_plotting_profile.m
...
sed -i 's/SED_xlim_SED/0 5/g' basic_plotting_profile.m # in nm
...
# Create the files that combine all supbfiles of a specific property
plotdata='integralcapacitance.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
do
	...
	legend=$legend' {\itT}='$temperature_name'K {\itU}_{drop}(V)'
	cp $plotdata tmp
	paste tmp 'integralcapacitance+'$temperature_name'.dat' > $plotdata
	rm tmp
...
# Create the files that combine all supbfiles of a specific property
plotdata='potentialdrop-smooth.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
do
	...
	legend=$legend' {\itT}='$temperature_name'K {\itU}_{drop}(V)'
	cp $plotdata tmp
	paste tmp 'potentialdrop-smooth+'$temperature_name'.dat' > $plotdata
	rm tmp
...
# Create the files that combine all supbfiles of a specific property
plotdata='reversedpotentialdrop-smooth.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
do
	...
	legend=$legend' {\itT}='$temperature_name'K \sigma^{\itT}-\sigma^{250K}(\muCcm^{-2})'
	cp $plotdata tmp
	paste tmp 'reversedpotentialdrop-smooth+'$temperature_name'.dat' > $plotdata
	rm tmp
