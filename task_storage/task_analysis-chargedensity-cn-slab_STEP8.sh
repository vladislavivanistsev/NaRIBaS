...
mkdir -p $dir_analysis/$fullpath2simulate/numberdens/temperature/
cd $dir_analysis/$fullpath2simulate/numberdens/temperature/
...
plotdata='numberdens+'$numberdens_name'-'$electrode_name'+'$surfacecharge_name'.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
sed -i 's|SED_title_SED|'$numberdens_name'-'$electrode_name' '$electrode_sign$surfacecharge_name' \\muC cm^{-2}|g' basic_plotting_profile.m
...
sed -i 's/\\rho_N(nm^{-3})/\\rho_N^{\\itT}-\\rho_N^{250K}(nm^{-3})/g' $plotdata
...
sed -i 's|SED_title_SED|'$numberdens_name'-'$electrode_name' '$electrode_sign$surfacecharge_name' \\muC cm^{-2}|g' basic_plotting_profile_difference.m
