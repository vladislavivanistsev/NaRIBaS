...
plotdata='ionnumber+'$numberdens_name'-'$electrode_name'+'$cutoff_name'.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
cp $plotdata tmp
paste tmp $dir_analysis/$fullpath2simulate/cutoff/'ionnumber+'$numberdens_name'-'$electrode_name'+'$temperature_name'+'$cutoff_name'.dat' > $plotdata
rm tmp
...
sed -i 's|SED_title_SED|'$numberdens_name'-'$electrode_name' cutoff='$cutoff_name'nm|g' basic_plotting_profile.m
