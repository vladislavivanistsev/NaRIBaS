...
mkdir -p $dir_analysis/$fullpath2simulate/chargedens/temperature/
cd $dir_analysis/$fullpath2simulate/chargedens/temperature/
...
plotdata='chargedens+'$electrode_name'+'$surfacecharge_name'.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
legend=$legend' '$temperature_name'K z(nm)'
cp $plotdata tmp
...
sed -i 's|SED_title_SED|'$electrode_name' '$surfacecharge_name' \\muC cm^{-2}|g' basic_plotting_profile.m
...
sed -i 's+\\rho_Q(e/nm^3)+\\rho_Q^{\\itT}-\\rho_Q^{250K}(e/nm^3)+g' $plotdata
...
sed -i 's|SED_title_SED|'$electrode_name' '$surfacecharge_name' \\muC cm^{-2}|g' basic_plotting_profile_difference.m
