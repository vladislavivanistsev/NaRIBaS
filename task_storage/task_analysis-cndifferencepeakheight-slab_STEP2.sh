...
cd $dir_analysis/$fullpath2simulate/numberdens/
...
plotdata='numberdens-cn+'$numberdens_name'-'$electrode_name'+difference+peakheight.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
legend='max(cn_N^{\sigma}-cn_N^{PZC}),1.Peak'
...
legend=$legend' T='$temperature_name'K {\itU}_{drop}(V)'
cp $plotdata tmp
paste tmp $dir_analysis/$fullpath2simulate/numberdens/'numberdens-cn+'$numberdens_name'-'$electrode_name'+'$temperature_name'+difference+peakheight.dat' > $plotdata
rm tmp
...
sed -i 's/SED_xlim_SED/0 7/g' basic_plotting_profile.m # in nm
...
sed -i 's/SED_plotfile_SED/'$plotdata'/g' basic_plotting_profile.m
sed -i 's|SED_title_SED|'$numberdens_name'-'$electrode_name'|g' basic_plotting_profile.m
sed -i 's/SED_savename_SED/close-up+'$plotdata'/g' basic_plotting_profile.m

sed -i 's/SED_verticallines_SED/-100/g' basic_plotting_profile.m # position in nm
#sed -i 's/SED_xlim_SED/x(1) x(end)/g' basic_plotting_profile.m # in nm
sed -i 's/SED_xlim_SED/0 2/g' basic_plotting_profile.m # in nm
...
# Create the files that combine all subfiles of a specific property
plotdata='numberdens-cn+'$numberdens_name'-'$electrode_name'+difference+peakheight+differentialresponse.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
# That is the legend. Should look like: "y plot1 x plot2 x plot3" if plotting is used later
legend='d(max(cn_N^{\sigma}-cn_N^{PZC}))/d{\itU}_{drop}'
...
legend=$legend' T='$temperature_name'K {\itU}_{drop}(V)'
cp $plotdata tmp
paste tmp $dir_analysis/$fullpath2simulate/numberdens/'numberdens-cn+'$numberdens_name'-'$electrode_name'+'$temperature_name'+difference+peakheight+differentialresponse.dat' > $plotdata
rm tmp
