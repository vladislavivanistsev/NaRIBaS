...
mkdir -p $dir_analysis/$fullpath2simulate/middleoftheslabrdf/temperature-dependence/
cd $dir_analysis/$fullpath2simulate/middleoftheslabrdf/temperature-dependence/
...
plotdata='rdf+'$rdf_name'+'$surfacecharge_name'.dat'
...
legend_1=$legend_1' '$temperature_name' r(nm)'
...
sed 's+SED_dir_scripts_SED+'$currentdir'/source+g' $currentdir/source/basic_plotting_xlim_ylim.m > basic_plotting.m
sed -i 's/SED_plotfile_SED/'$plotdata'/g' basic_plotting.m
sed -i 's/SED_title_SED/'$rdf_name' \\sigma='$surfacecharge_name'\\muC cm^{-2}/g' basic_plotting.m
sed -i 's/SED_savename_SED/'$plotdata'.dat/g' basic_plotting.m
sed -i 's/SED_xlim_SED/0 4/g' basic_plotting.m
sed -i 's/SED_ylim_SED/0 2/g' basic_plotting.m
