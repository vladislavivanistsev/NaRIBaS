...
sed 's+SED_dir_scripts_SED+'$currentdir'/source+g' $currentdir/source/ion_number_at_cutoff.m > ion_number_at_cutoff.m
sed -i 's/SED_cnfile_SED/'$plotdata'/g' ion_number_at_cutoff.m
sed -i 's|SED_potentialdropfile_SED|'$dir_analysis'/'$fullpath2simulate'/potentialdrop/potentialdrop-smooth+'$temperature_name'.dat|g' ion_number_at_cutoff.m
sed -i 's/SED_savename_SED/ionnumber+'$numberdens_name'-'$electrode_name'+'$temperature_name'+'$cutoff_name'.dat/g' ion_number_at_cutoff.m
...
# Run matlab
$matlabdir/matlab -nodisplay -nosplash -r  "ion_number_at_cutoff($ncols,$cutoff_name,$r_ion_nm,$r_wall_nm,$xbox_nm,$ybox_nm,$currentdens)"
