...
plotdata='chargedens+'$duration_name'+ion-electrode.dat'
if [ -e $plotdata ]; then
  rm $plotdata
fi
touch $plotdata
...
legend='\rho_{Q}(e/nm^3)'

while read inputline
do
electrode_name=$(echo $inputline | awk '{print $1}')

legend=$legend' '$electrode_name' z(nm)'
cp $plotdata tmp
...
factor=$(cat $currentdir/$inputlists_folder/electrode.list | wc -l)
ncols=$(($factor*2))
