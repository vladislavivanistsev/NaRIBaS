...
mkdir -p $dir_analysis/$fullpath2simulate/potentialdrop/
cd $dir_analysis/$fullpath2simulate/potentialdrop/
...
electrode_name=$(echo $inputline | awk '{print $1}')
	...
	plotdata='chargedens+'$electrode_name'+'$temperature_name'.dat'
	if [ -e $plotdata ]; then
	  rm $plotdata
	fi
	touch $plotdata
	...
	do
		...
		legend=$legend' '$surfacecharge_name' z(nm)'
		cp $plotdata tmp
		paste tmp $dir_analysis/$fullpath_allreplicas/'chargedens+'$duration_name'+'$electrode_name'.dat' > $plotdata
		rm tmp

	done < $currentdir/$inputlists_folder/surfacecharge.list  ######## WARNING: may be changed to surfacecharge.list ############
...
factor=$(cat $currentdir/$inputlists_folder/surfacecharge.list | wc -l)  ######## WARNING: may be changed to surfacecharge.list ############
ncols=$(($factor*2))


# Edit the Matlab function to calculate the potential drop
sed 's+SED_dir_scripts_SED+'$currentdir'/source+g' $currentdir/source/potential_drop.m > potential_drop.m
sed -i 's/SED_anode-file_SED/chargedens+Anode+'$temperature_name'.dat/g' potential_drop.m
sed -i 's/SED_cathode-file_SED/chargedens+Cathode+'$temperature_name'.dat/g' potential_drop.m
sed -i 's/SED_savename_SED/potentialdrop+'$temperature_name'.dat/g' potential_drop.m

# Run matlab
$matlabdir/matlab -nodisplay -nosplash -r  "potential_drop($ncols)"

# Edit the Matlab function to calculate the capacitance
sed 's+SED_dir_scripts_SED+'$currentdir'/source+g' $currentdir/source/Uvssigma2capacitance.m > Uvssigma2capacitance.m

sed -i 's/SED_file_SED/potentialdrop+'$temperature_name'.dat/g' Uvssigma2capacitance.m
sed -i 's/SED_savename-smoothpotentialdrop_SED/potentialdrop-smooth+'$temperature_name'.dat/g' Uvssigma2capacitance.m
sed -i 's/SED_savename_SED/capacitance+'$temperature_name'.dat/g' Uvssigma2capacitance.m

# Run matlab
$matlabdir/matlab -nodisplay -nosplash -r  "Uvssigma2capacitance()"
