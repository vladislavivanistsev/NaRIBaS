...
layerdata_name=$(echo ${current_layerdata_normalized[0]} | awk '{print $1}')
layerdata_legend=$(echo ${current_layerdata_normalized[0]} | awk '{print $2}')
...
sed 's+SED_dir_scripts_SED+'$currentdir'/source+g' $currentdir/source/basic_plotting_profile_long_legendoutside_inset.m > basic_plotting_profile_long_legendoutside_inset.m
sed -i 's/SED_plotfile_SED/'$plotdata'/g' basic_plotting_profile_long_legendoutside_inset.m
sed -i 's|SED_title_SED|'$numberdens_name' '$layerdata_name'|g' basic_plotting_profile_long_legendoutside_inset.m
sed -i 's/SED_savename_SED/inset+'$plotdata'/g' basic_plotting_profile_long_legendoutside_inset.m

sed -i 's/SED_verticallines_SED/0/g' basic_plotting_profile_long_legendoutside_inset.m # position in nm
sed -i 's/SED_xlim_SED/-7 7/g' basic_plotting_profile_long_legendoutside_inset.m # in nm
#sed -i 's/SED_legendlocation_SED/'$numberdens_legendlocation'/g' advanced_plotting_profile_long_noylim.m # in nm

# Run matlab
$matlabdir/matlab -nodisplay -nosplash -r  "basic_plotting_profile_long_legendoutside_inset($ncols)"
