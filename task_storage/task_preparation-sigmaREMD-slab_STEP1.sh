...
temperature_name=$(echo ${current_temperature_sigma_REMD[0]} | awk '{print $1}')
version_name=$(echo ${current_version[0]} | awk '{print $1}')
surfacecharge_name=$(echo ${current_surfacecharge_sigma_REMD[0]} | awk '{print $1}')
surfacecharge_temperaturevariation=$(echo ${current_surfacecharge_sigma_REMD[0]} | awk '{print $2}')
...
cp $dir_experiments/$fullpath_referencegro/NVT.gro reference.gro
...
make_ndx -f reference.gro -o index_slab.ndx << EOF
keep 0
a $cation_name
name 1 Cation
a $anion_name
...
exp="$temperature_name+$surfacecharge_temperaturevariation"
modifiedtemperature=$(awk "BEGIN {print $exp}" /dev/null)

sed 's/SED_temperature_name_SED/'$modifiedtemperature'/g' $dir_systempreparation/5_NVT_final_Slab_emptyTemp.mdp > 5_NVT_final_Slab.mdp
...
grompp -f 5_NVT_final_Slab.mdp -c reference.gro -p topol_local_charged.top -n index_slab.ndx -o NVT -maxwarn 1
rm mdout.mdp
...
numofwalkers=$(cat $currentdir/$inputlists_folder/surfacecharge_sigma_REMD.list | wc -l)

while [ $runningnumber -ge $numofwalkers ]; do
runningnumber=$((runningnumber - numofwalkers))
done

# copy tpr files to run them somewhere as multijobs
cp NVT.tpr $dir_experiments/$fullpath2simulate/NVT_$temperature_name\_$runningnumber.tpr
...
pathtocopyback=$(pwd)
echo "cp NVT_$temperature_name\_$runningnumber.tpr $pathtocopyback/NVT.tpr" >> $dir_experiments/$fullpath2simulate/copyback.txt
