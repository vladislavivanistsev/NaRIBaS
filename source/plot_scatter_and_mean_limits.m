...
xSize = 17.2; ySize = 7.0; %For APS paper the recomended figure width equals the column width of the paper 8.6 cm (or between 10 cm and 2 times 8.6 cm for long figures)
...
for m = 1:2:ncols
  ...
  if (m==1)
    h=plot(x,y(:,m),'o','Linewidth',1);
  elseif (m==ncols-1)
    h=plot(y(:,m-1),y(:,m),'-','Linewidth',1);
  else
    h=plot(y(:,m-1),y(:,m),'o','Linewidth',1);
  end
