...
# Remove temperature.list, this one will now be considered as input loop in the tasks
inputlists=(particle.list cation.list anion.list impurity.list version.list boxsize.list duration_NVT.list rdf.list)

####### Add here the file that contains the task list
tasks=task_analysis-rdf-bulk_STEP3.sh
